...
def nii_file():
    ...
    if not nii_file:
        ...
        if not xlsx_path:
            ...
            with open('/storage/' + nii_file_name +'.pickle', 'wb') as f:
                pickle.dump(final_df_ID, f)
            ###########################################################################################
            np.save('/storage/' + nii_file_name +'.npy', img_new) #保存為.npy  使用時標註解開
            ##########################################################################################
            x = np.load('/storage/' + nii_file_name +'.npy')
            x = x[1:,:,:,:]
            np.save('/storage/' + nii_file_name +'.npy',x)
            messagebox.showinfo("Saving Successful", ('Pickle and Npy Saving at'+ '/storage/' ))
###########################################################################################
###########################################################################################
###########################################################################################
    ...
    if var_class == 'BrainAge':
        ...
        if not pickle_path:
            ...
            if not file_path:
                ...
                df_da.to_csv('/storage/' +'BrainAge.csv',index = False)
        ...
        if not pickle_path:
            ...
            if not file_path:
                ...
                df_da.to_csv('/storage/' +'NIFD_2ndclass.csv',index = False)
                messagebox.showinfo("Prediction Result", "Predict Success")
    ####################################################################
    ####################################################################
        ...
        if not pickle_path:
            ...
            if not file_path:
                ...
                df_da.to_csv('/storage/' +'ADNI&NIFD_classification.csv',index = False)
                messagebox.showinfo("Prediction Result","Predict Success")
    #######################################################################################
    #######################################################################################
        ...
        if not pickle_path:
            ...
            if not file_path:
                ...
                df_da.to_csv('/storage/'+'CDRSB_6class.csv',index = False)
                messagebox.showinfo("Prediction Result", "Predict Success")
        ...
        if not pickle_path:
            ...
            if not file_path:
                ...
                df_da.to_csv('/storage/'+'CDRSB_6reg.csv',index = False)
                messagebox.showinfo("Prediction Result", "Predict Success")
        ...
        if not pickle_path:
            ...
            if not file_path:
                ...
                df_da.to_csv('/storage/'+'3111class.csv',index = False)
                messagebox.showinfo("Prediction Result", "Predict Success")
        ...
        if not pickle_path:
            ...
            if not file_path:
                ...
                df_da.to_csv('/storage/'+'2112class.csv',index = False)
                messagebox.showinfo("Prediction Result", "Predict Success")
        ...
        if not pickle_path:
            ...
            if not file_path:
                ...
                df_da.to_csv('/storage/'+'CN_MCI_other.csv',index = False)
                messagebox.showinfo("Prediction Result", "Predict Success")
    else:
